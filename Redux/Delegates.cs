...
namespace Redux
{
    ...
    /// <returns>A function that, when called with a <see cref="Dispatcher" />, returns a new
    ///     <see cref="Dispatcher" /> that wraps it.</returns>
    ...
    /// <summary>
    ///     Represents a method that can be used as an async listener by
    ///     <see cref="T:ListenerMiddleware{TState}" />.
    /// </summary>
    /// <typeparam name="TState">The state tree type.</typeparam>
    ...
    /// <param name="dispatcher">The dispatcher that the listener can use to dispatch new actions to the store.</param>
    public delegate void AsyncListener<TState>([CanBeNull] object action, [CanBeNull] TState state, Dispatcher dispatcher);
}